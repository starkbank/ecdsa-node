...
var BinaryAscii = require("./binary");
var modulo = require("./integer").modulo;
...
var bytesHex0 = Buffer.from(hex0).toString('binary');
var bytesHexB = Buffer.from(hexB).toString('binary');
var bytesHexC = Buffer.from(hexC).toString('binary');
var bytesHexD = Buffer.from(hexD).toString('binary');
var bytesHexF = Buffer.from(hexF).toString('binary');
...
exports.removeSequence = function (string) {
    ...
    return [string.slice(1 + lengthLen, endSeq), string.slice(start=endSeq)];
}


exports.removeInteger = function (string) {
    _checkSequenceError(string=string, bytesHexB, "02");
...
exports.removeBitString = function (string) {
    ...
    let body = string.slice(start=1 + lengthLen, 1 + lengthLen + length);
...
exports.removeOctetString = function (string) {
    _checkSequenceError(string=string, bytesHexD, "04");

    let result = _readLength(string.slice(start=1));
    let length = result[0];
    let lengthLen = result[1];

    body = string.slice(start=1 + lengthLen, 1+lengthLen+length);
