...
import * as File from "../ellipticcurve/utils/file";
...
describe("ECDSA test", function () {
  ...
  describe("#testZeroSignature()", function () {
    it("should deny authenticity", function () {
      ...
      assert.equal(
        Ecdsa.verify(message, new Signature(bigInt(0), bigInt(0)), publicKey),
        false
      );
    });
  });
...
describe("openSSL test", function () {
  describe("#testAssign()", function () {
    it("should read and verify PEM file", function () {
      ...
      let privateKeyPem = File.read(
        path.join(TEST_DIR, "test", "privateKey.pem")
      );
  ...
  describe("#testVerifySignature()", function () {
    it("should read and verify signature file", function () {
      ...
      let publicKeyPem = File.read(
        path.join(TEST_DIR, "test", "publicKey.pem")
      );
      // openssl dgst -sha256 -sign privateKey.pem -out signature.binary message.txt
      let signatureDer = File.read(
        path.join(TEST_DIR, "test", "signatureDer.txt"),
        "binary"
      );
      let message = File.read(path.join(TEST_DIR, "test", "message.txt"));
...
describe("Signature test", function () {
  ...
  describe("#testExternalRandNum()", function () {
    it("should confirm authenticity and same signature", function () {
      ...
      let signature1 = Ecdsa.sign(message, privateKey, undefined, bigInt(123));
      let signature2 = Ecdsa.sign(message, privateKey, undefined, bigInt(123));

      assert.equal(Ecdsa.verify(message, signature1, publicKey), true);
      assert.equal(Ecdsa.verify(message, signature2, publicKey), true);
      assert.equal(signature1.r.toString(), signature2.r.toString());
      assert.equal(signature1.s.toString(), signature2.s.toString());
    });
  });
});
