...
function calculateParameters(range: BigInteger): Parameters {
...
export function between(minimum: BigInteger, maximum: BigInteger): BigInteger {
  ...
  for (let i = bigInt(0); i.lesser(bytesNeeded); i = i.add(1)) {
    randomValue = randomValue.or(
      bigInt(randomBytes[i.toJSNumber()]).shiftLeft(bigInt(8).multiply(i))
    );
  }
